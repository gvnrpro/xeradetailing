...
const MobileNavBar = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-black/90 backdrop-blur-sm border-t border-xera-lightgray md:hidden">
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <motion.div
                whileTap={{ scale: 0.9 }}
                className={`flex flex-col items-center justify-center ${isActive ? 'text-xera-red' : 'text-white/70'}`}
              >
                {item.icon}
                ...
                {isActive && (
                  <motion.div 
                    ...
                    className="absolute bottom-0 w-10 h-0.5 bg-xera-red"
                    initial={false}
